...
changelog[0.3] = "Added NAS Location for auto insertion of location, prefix location for prefix of filename to replace"

# ================================================

# User variables to set location of NAS Server and prefix of iTunes filenames for auto-replacement

NASlocation = 'mnt/NAS/Test'
iTunesPrefix = 'D:\Music\iTunes\iTunes Media\Music'
...
def gen_extended(data):
    ...
    for n in range(int(len(data)/2)):
        ...
        try:
            ...
            uri = uri.encode('ascii').decode('utf-8').replace((iTunesPrefix), (NASlocation)).replace('\\', '/')
            ...
            uri = uri.encode('ascii', 'ignore').decode('utf-8').replace((iTunesPrefix), (NASlocation)).replace('\\', '/')
